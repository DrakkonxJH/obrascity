# Auditoria Master - Go-Live ObrasFlow
...
| Domínio `obrasflow.com` verificado no Resend | NÃO | Painel mostrou `Status: Pendente` | Configurar DNS correto |
| DNS público do domínio existente (NS/A) | NÃO | `dig` sem resposta para `obrasflow.com` | Corrigir delegação DNS |
...
1. Resolver DNS autoritativo do `obrasflow.com`.
